...
size_t hmac_sha256(const void* key,
    ...
    if (keylen > SHA256_BLOCK_SIZE) {
        ...
        memcpy(k, key, keylen);
        for (register size_t i = keylen; i < SHA256_BLOCK_SIZE; i++) {
            k[i] = '\0';
        }
    }
...
size_t hmac_sha384(const void* key,
    const size_t keylen,
    const void* data,
    const size_t datalen,
    void* out,
    const size_t outlen) {
    uint8_t k[SHA384_BLOCK_SIZE];
    uint8_t k_ipad[SHA384_BLOCK_SIZE];
    uint8_t k_opad[SHA384_BLOCK_SIZE];
    uint8_t ihash[SHA384_HASH_SIZE];
    uint8_t ohash[SHA384_HASH_SIZE];
    size_t sz;
    int i;

    memset(k, 0, sizeof(k));
    memset(k_ipad, 0x36, SHA384_BLOCK_SIZE);
    memset(k_opad, 0x5c, SHA384_BLOCK_SIZE);

    if (keylen > SHA384_BLOCK_SIZE) {
        // If the key is larger than the hash algorithm's
        // block size, we must digest it first.
        sha384(key, keylen, k, sizeof(k));
    }
    else {
        memcpy(k, key, keylen);
        for (register size_t i = keylen; i < SHA384_BLOCK_SIZE; i++) {
            k[i] = '\0';
        }
    }
    ...
    // Perform HMAC algorithm: ( https://tools.ietf.org/html/rfc2104 )
    //      `H(K XOR opad, H(K XOR ipad, data))`
    H_384(k_ipad, sizeof(k_ipad), data, datalen, ihash, sizeof(ihash));
    H_384(k_opad, sizeof(k_opad), ihash, sizeof(ihash), ohash, sizeof(ohash));

    sz = (outlen > SHA384_HASH_SIZE) ? SHA384_HASH_SIZE : outlen;
    memcpy(out, ohash, sz);
    return sz;
}
...
static void* H_384(const void* x,
    ...
    memcpy(buf, x, xlen);
    memcpy(buf + xlen, y, ylen);
    result = sha384(buf, buflen, out, outlen);
